...
@property (nonatomic, strong) NSEntityDescription *personneEntite;
@property (nonatomic, strong) NSDictionary *attributsEntite;

@property (nonatomic) BOOL enPause, majListe, telechargementSondage, majCalendrier, changementTrombi;

-(void)identification;
-(void)recupTout;
-(NSArray *)personnesPourUsername:(NSString *)username;
...
-(ReseauNouveau *)reseau {
    if (!_reseau) {
        _reseau = [[ReseauNouveau alloc] init];
        [_reseau setDelegue:self];
    }
    return _reseau;
}

-(NSEntityDescription *)personneEntite {
    if (!_personneEntite) _personneEntite = [NSEntityDescription entityForName:@"Personne" inManagedObjectContext:self.context];
    return _personneEntite;
}

-(NSDictionary *)attributsEntite {
    if (!_attributsEntite) _attributsEntite = [self.personneEntite attributesByName];
    return _attributsEntite;
}
...
-(void)renvoieInfos:(NSDictionary *)dico forUsername:(NSString *)personne {
    ...
    if (dico) {
        ...
        NSArray *trouve = [self personnesPourUsername:personne];
        if ([trouve count] == 1) {
            Personne *personne = [trouve objectAtIndex:0];
            for (NSString *attribute in self.attributsEntite) {
                id value = [dico objectForKey:attribute];
                if (value == nil) {
                    continue;
                }
                [personne setValue:value forKey:attribute];
            }
        }
...
-(void)sauvegardeTrombi:(NSArray *)trombi {
    ...
    NSLog(@"a");
    NSFetchRequest *requete = [NSFetchRequest fetchRequestWithEntityName:@"Personne"];
    [requete setPropertiesToFetch:[NSArray arrayWithObject:@"username"]];
    [requete setResultType:NSDictionaryResultType];
    NSArray *membres = [self.context executeFetchRequest:requete error:NULL];
    NSMutableArray *tab = [[NSMutableArray alloc] initWithCapacity:[membres count]];
    for (NSDictionary *membre in membres) {
        [tab addObject:[membre objectForKey:@"username"]];
    }
    membres = nil;
    
    for (NSDictionary *dico in trombi) {
        if (![tab containsObject:[dico objectForKey:@"username"]]) {
            Personne *personne = [NSEntityDescription insertNewObjectForEntityForName:@"Personne" inManagedObjectContext:self.context];
            for (NSString *attribute in self.attributsEntite) {
                id value = [dico objectForKey:attribute];
                if (value == nil) {
                    continue;
...
-(NSArray *)personnesPourUsername:(NSString *)username {
    NSFetchRequest *requete = [NSFetchRequest fetchRequestWithEntityName:@"Personne"];
    [requete setPredicate:[NSPredicate predicateWithFormat:@"username LIKE %@",username]];
    return [self.context executeFetchRequest:requete error:NULL];
}
...
-(NSArray *)getTrombi {
    if (!self.trombi) {
        ...
        NSFetchRequest *requete = [NSFetchRequest fetchRequestWithEntityName:@"Personne"];
        [requete setPropertiesToFetch:[NSArray arrayWithObjects:@"username", @"last_name", @"first_name", @"promo", @"phone", nil]];
        [requete setResultType:NSDictionaryResultType];
        [requete setSortDescriptors:[NSArray arrayWithObject:[NSSortDescriptor sortDescriptorWithKey:@"last_name" ascending:YES]]];
        NSArray *result = [self.context executeFetchRequest:requete error:NULL];
...
-(NSDictionary *)getInfos:(NSString *)identifiant etTelechargement:(BOOL)telechargement {
    if (!telechargement) {
        ...
        if (!dico) {
            NSFetchRequest *requete = [NSFetchRequest fetchRequestWithEntityName:@"Personne"];
            [requete setPredicate:[NSPredicate predicateWithFormat:@"username LIKE %@",identifiant]];
            [requete setResultType:NSDictionaryResultType];
            NSArray *personnes = [self.context executeFetchRequest:requete error:NULL];
            
            if (personnes && [personnes count]) {
                dico = [personnes objectAtIndex:0];
